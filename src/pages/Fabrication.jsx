import "../assets/CSS_JS/Fabrication.css";
...
const Fabrication = () => {
  ...
  // Charger tous les artisans puis filtrer ceux en "Fabrication"
  useEffect(() => {
    setLoading(true);
    fetch(`http://localhost:5000/api/Artisans?categorie=Fabrication`)
      ...
      .then((data) => {
        ...
        const filtered = normalized.filter((a) => {
          ...
          return cat.trim().toLowerCase() === "fabrication";
        });
  ...
  const handleSearchSubmit = (e) => {
    ...
    if (["alimentation", "bâtiment", "batiment", "services", "fabrication"].includes(query)) {
      navigate(`/${query.charAt(0).toUpperCase() + query.slice(1)}`);
      return;
    }

    // Redirection vers une fiche artisan
    const foundArtisan = artisans.find(
      (a) => a.nom.toLowerCase() === query
    );
    if (foundArtisan) {
      navigate(`/Fiche_artisan/${foundArtisan.id}`);
      return;
    }

    // Sinon, redirection vers Notfound
    navigate("/Notfound");
  };
  ...
  return (
    <div className="App">
      ...
      <Helmet>
        ...
        <meta
          name="description"
          content="Cette page est dédiée aux artisans et ou entreprises dans le domaine de la Fabrication "
        />
        <meta
          name="keywords"
      ...
      <main>
        <h1 className="titre-principal">Artisans du domaine Fabrication</h1>
        {loading ? (
          <p>Chargement…</p>
        ) : artisans.length === 0 ? (
